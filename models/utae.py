...
class UTAE(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            str_conv_s (int): Stride of strided convolutions.
            str_conv_p (int): Padding of strided convolutions.
            agg_mode (str): Aggregation mode ('att_group', 'att_mean', 'mean').
            encoder_norm (str): Normalization type ('group', 'batch', 'instance').
            n_head (int): Number of attention heads in LTAE.
            d_model (int): Model dimension in LTAE.
            d_k (int): Key-Query space dimension.
            encoder (bool): If True, return feature maps instead of class scores.
            return_maps (bool): If True, return intermediate feature maps.
            pad_value (float): Padding value for temporal padding.
            padding_mode (str): Spatial padding strategy.
        """
        ...
        self.in_conv = ConvBlock(
            nkernels=[self.in_channels] + [encoder_widths[0], encoder_widths[0]],
            pad_value=pad_value,
            norm=encoder_norm,
    ...
    def forward(self, x, return_att=False):
        ...
        Args:
            ...
            return_att: bool, whether to return attention maps

        Returns:
            torch.Tensor: segmentation map [B, num_classes, H, W]
            or tuple: (segmentation, attention) if return_att=True
            or dict: {"segmentation": [B, num_classes, H, W], "maps": [...]} if return_maps=True
        """
        # Input validation
        ...
        # Create pad mask (assuming no padding)
        ...
        # Temporal encoder (no batch_positions)
        ...
        if self.encoder:
            return out  # Return feature maps directly

        out = self.out_conv(out)

        if return_att:
            return out, att  # Return tuple (segmentation, attention)
        if self.return_maps:
            return out, maps  # Return tuple (segmentation, maps)

        return out  # Return segmentation tensor only
