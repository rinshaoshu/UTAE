...
预期输出: (2, 2, 128, 128)  双通道 softmax
"""
import torch
import torch.nn as nn


def check_model(name, model, input_tensor, expected_ch=2, forward_kwargs=None):
    """检查单个模型的输入输出。"""
    print(f"\n{'='*60}")
    print(f"[{name}]")
    ...
    try:
        ...
        with torch.no_grad():
            if forward_kwargs:
                output = model(input_tensor, **forward_kwargs)
            else:
                output = model(input_tensor)
        print(f"  输出形状: {tuple(output.shape)}  ✓")
        # 额外检查
        B, C, H, W = 2, expected_ch, 128, 128
...
def main():
    ...
    print(f"设备: {device}")
    print(f"统一输入形状: (2, 10, 15, 128, 128)  [B, T, C, H, W]")
    print(f"预期输出形状: (2, 2, 128, 128)     [B, num_classes, H, W]  双通道 softmax")
    ...
    model = UTAE(in_channels=15, num_classes=2).to(device)
    ...
    model = SwinUNetHeadWithTemporal(
        img_size=128, in_channels=15, num_classes=2
    ).to(device)
    ...
    model = ConvGRU_Seg(
        num_classes=2, img_res=128, in_channels=15,
